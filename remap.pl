...
my ($iv, $ov, $il, $ol, $ow, $oh, $ofs, $ovs, $dflag, $crf, $ecoef, $cbr, $ffpath); # input video, output video, input layout, output layout, output width, output height, output fragment shader, output vertex shader, debug flag
...
say "iv=$iv, ov=$ov, il=$il, ol=$ol, ow=$ow, oh=$oh, ofs=$ofs, ovs=$ovs, dflag=$dflag, crf=$crf, ecoef=$ecoef cbr=$cbr ffpath=$ffpath" if $dflag eq "debug";
...
unless($iv =~ /(\.jpg|\.bmp|\.png)/){
    $fps = $1 if `${ffpath}/ffprobe $iv 2>&1` =~ /([^\s]+)\s+fps/;
    $fps = int($fps+0.5);
